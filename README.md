🏦 Bank Loan Performance and Risk Analysis Report

View Interactive Dashboard

Project Overview

This project focuses on creating a comprehensive Bank Loan Report to monitor and assess the bank's lending activities and portfolio performance. By analyzing key loan-related metrics over time, the report enables data-driven decision-making, tracks the overall health of the loan portfolio, and identifies critical trends that inform future lending strategies and risk management.

The final output is an interactive dashboard built using Power BI, designed for executive review and operational oversight.

🎯 Key Performance Indicators (KPIs)

The report tracks the following essential metrics to measure lending activity and risk:

Metric

Category

Total Loan Application

Volume

Total Funded Amount

Capital Deployed

Total Amount Received

Collection Performance

Average Interest Rate

Cost of Capital

Average Debt-to-Income Ratio (DTI)

Risk Assessment

Good Loan / Bad Loan

Portfolio Health (Classification)

📊 Rationale for Analysis

Analyzing bank loan data is crucial for maintaining financial stability and maximizing profitability. This report provides the necessary insights for:

Risk Assessment: Evaluating the probability of default for new and existing loans.

Decision-Making: Setting appropriate interest rates, loan amounts, and approval criteria.

Portfolio Management: Identifying segments with high risk or high performance for strategic adjustments.

Credit Risk Management: Implementing models to minimize losses from non-performing loans.

Regulatory Compliance: Ensuring adherence to financial regulations and reporting standards.

💾 Data Dictionary (Fields Used)

The analysis is based on a dataset containing the following fields:

Field Name

Description

Loan ID

Unique identifier for each loan application.

Address State

Borrower's geographic location.

Employee Length

Duration of the borrower's employment (used to gauge stability).

Employee Title

Borrower's occupation or job title.

Grade

Primary risk classification based on creditworthiness (Higher = Lower Risk).

Sub Grade

Refined risk assessment within the primary grade.

Home Ownership

Borrower's housing status (e.g., Mortgage, Rent, Own).

Issue Date

The date the loan was originated.

Last Payment Date

Date of the most recent loan payment received.

Loan Status

Current state of the loan (e.g., Fully Paid, Current, Default).

Purpose

The stated reason for the loan (e.g., debt consolidation, business).

Term

The duration of the loan in months (repayment period).

Verification Status

Status of financial information verification.

Annual Income

Borrower's total yearly earnings.

DTI (Debt-to-Income Ratio)

Measures debt burden relative to income.

Instalment

Fixed monthly payment amount.

Interest Rate

Annual cost of borrowing (APR).

Loan Amount

The total principal sum borrowed.

🛠️ Technology Stack

This project was developed using:

Tool: Microsoft Power BI

Purpose: Data ingestion, cleaning, transformation (Power Query/DAX), and visualization development.

✨ Key Project Learnings & Skills

This project demonstrated proficiency in end-to-end business intelligence workflow:

Dashboard Development: Created an interactive, user-friendly dashboard to track and analyze lending performance in real-time.

Data Modeling & Transformation (ETL): Established connections, joined multiple data tables, and performed complex calculations (DAX/M) to derive key metrics.

Data Customization: Implemented user-driven parameters, filters, and slicers to enable drill-down functionality within the analysis.

Visualization Diversity: Utilized a variety of customized visualizations (bar, pie, donut, clustered, scatter, line, area charts, and maps) to present insights effectively.
